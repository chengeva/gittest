...
import co.acaia.communications.Scale;
...
//        CommLogger.logv(TAG, "Testing parse packet length=" + sdata.length);
    ...
    public static void parse_eventmsg(app_prsdata o_data,Struct.Unsigned8[] s_param, byte [] orig_data, Context context, boolean isCinco){
        ...
        while(ln_length>0){
            ...
            if(ls_process== co.acaia.communications.protocol.ver20.ScaleProtocol.EEVENT.e_appevent_weight.ordinal()){
                ...
                float weightVal =0;
                int unit=(int)o_data.curr_unit.get();
                if(unit==5){
                    unit=4;
                }
                ...
                int dp=0;
                //Log.v("test","Cinco"+String.valueOf(isCinco)+" "+String.valueOf(dp)+" "+String.valueOf(unit));
                if(isCinco &&unit==2){
                    dp=2;
                }
                if(isCinco && unit==4){
                ...
                if(!isCinco){
                    unit=wtevent.getUnit();
                }

                //CommLogger.logv(TAG,"weight dp="+String.valueOf(dp));
                if(unit==4 )
                    weightVal =(float)(val/10000.0);
                else if(unit==2 || unit==0) {
                ...
                if (unit == 0) {
                    // gram
                    weightString = String.format("%.21f", weightVal);
                ...
                if(!illegalDataDetected) {
                    //Log.v(TAG, "weight string=" + weightString);
...
//        CommLogger.logv(TAG, "n_event: " + String.valueOf(n_event))
        if(n_event== ScaleProtocol.ECMD.new_cmd_sync_brewguide_cmd.ordinal()){

            Log.v("Brewguide","Scale want data...");
            short scale_page=(short)(s_param[2].get()<<8 | s_param[1].get());
            BrewguideScaleEvent brewguideScaleEvent=new BrewguideScaleEvent(s_param[0].get(),scale_page);
            EventBus.getDefault().post(brewguideScaleEvent);

        }else if(n_event==ScaleProtocol.ECMD.e_cmd_info_a.ordinal()){
            Log.v(TAG, "n_event=e_cmd_info_a");
            ...
            EventBus.getDefault().post(new FirmwareEvent(scale_info_.getMainVersion(),scale_info_.getSubVersion(),scale_info_.getInfoVersion()));
            scale_info_.printDebug();
            ...
            Log.v(TAG, "n_event=e_cmd_status_a");
            ...
            o_data.curr_unit.set((scaleStatus.n_unit.get()));
            PearlSStatusEvent pearlSStatusEvent=new PearlSStatusEvent(scaleStatus);
            EventBus.getDefault().post(pearlSStatusEvent);
            ...
            parse_eventmsg(o_data,s_param,orig_data,context,isCinco);
        ...
        if(o_data.mn_app_index.get()==o_data.mn_app_len.get()){
            ...
            if (o_data.mn_appstep.get() ==  ScaleProtocol.EAPP_PROCESS.e_prs_checksum.ordinal())
            {
                ...
                if (o_data.mn_app_checksum.get() == o_data.mn_app_datasum.get()) {
                    ...
                    if (isSette) {
                        FellowPacketParser.app_event(o_data,o_data.mn_id,o_data.mn_app_cmdid.get(),o_data.mn_app_buffer,orig_data,context);
                       // Log.v("DataPacketParser","Fello Event!");
                    } else {
