...
public class DataOutHelper {
    ...
    public static byte[] setting_chg(short n_item,short n_value){
        ...
        return u1_array_to_byte_array_with_len(out.ls_out,ln_count);
    }

    public static  byte[] send_brewguide_info(BrewguideProtocol.new_brewguide_data_info brewguide_data_info){
        output_struct out = new output_struct();
        sr_len_struct srLenStruct = new sr_len_struct();
        srLenStruct.sr_len.set((short) 8);
        int ln_count = pack_data(out.ls_out, (short) ScaleProtocol.ECMD.new_cmd_sync_brewguide_data_info.ordinal(), brewguide_data_info.getByteArray(), srLenStruct.sr_len);

        return u1_array_to_byte_array_with_len(out.ls_out,ln_count);
    }

    public static  byte[] bt2SendBrewguideStr(BrewguideProtocol.new_brewguide_data_string brewguide_data_string){
        output_struct out = new output_struct();
        sr_len_struct srLenStruct = new sr_len_struct();
        srLenStruct.sr_len.set((short) 14);
        int ln_count = pack_data(out.ls_out, (short) ScaleProtocol.ECMD.new_cmd_sync_brewguide_data_str.ordinal(), brewguide_data_string.getByteArray(), srLenStruct.sr_len);

        return u1_array_to_byte_array_with_len(out.ls_out,ln_count);
    }


    public static  byte[] send_brewguide_step(BrewguideProtocol.new_brewguide_data_step brewguide_data_step){
        output_struct out = new output_struct();
        sr_len_struct srLenStruct = new sr_len_struct();
        srLenStruct.sr_len.set((short) 12);
        int ln_count = pack_data(out.ls_out, (short) ScaleProtocol.ECMD.new_cmd_sync_brewguide_data_type.ordinal(), brewguide_data_step.getByteArray(), srLenStruct.sr_len);
        return u1_array_to_byte_array_with_len(out.ls_out,ln_count);
    }


    public static byte[] send_brewguide_cmd(short n_item,short n_value){
        output_struct out = new output_struct();
        BrewguideProtocol.new_brewguide_setting brewguide_setting=new BrewguideProtocol.new_brewguide_setting(n_item,n_value);
        sr_len_struct srLenStruct = new sr_len_struct();
        srLenStruct.sr_len.set((short) 3);
        int ln_count = pack_data(out.ls_out, (short) ScaleProtocol.ECMD.new_cmd_sync_brewguide_cmd.ordinal(), brewguide_setting.getByteArray(), srLenStruct.sr_len);
        // debug_byte("send setting",u1_array_to_byte_array(out.ls_out));
        return u1_array_to_byte_array_with_len(out.ls_out,ln_count);
    }
    ...
    public static void default_event(){
        ...
        // hanjord test close data
        ln_count+=pack_event(ls_data.ls_data,ln_count, (short)(ScaleProtocol.EEVENT.e_scalevent_weight.ordinal()),(short)10);
        ln_count+=pack_event(ls_data.ls_data,ln_count, (short)(ScaleProtocol.EEVENT.e_scalevent_battery.ordinal()),(short)60);
        ln_count+=pack_event(ls_data.ls_data,ln_count, (short)(ScaleProtocol.EEVENT.e_scalevent_timer.ordinal()),(short)60);
        ...
        int count=pack_data(out.ls_out,  (short)(ScaleProtocol.ECMD.e_cmd_event_sa.ordinal()), ls_data.ls_data, srLenStruct.sr_len);
        EventBus.getDefault().post(new SendDataEvent(u1_array_to_byte_array_with_len(out.ls_out,count)));
    ...
    public static Boolean sent_appid(ScaleProtocol.app_prsdata o_data,byte[] s_id){
        ...
        int ln_count=pack_data(out.ls_out, (short) ScaleProtocol.ECMD.e_cmd_identify_s.ordinal(), s_out.ls_out, sr_len.sr_len);
        EventBus.getDefault().post(new SendDataEvent(u1_array_to_byte_array_with_len(out.ls_out,ln_count)));
        return true;

    }

    public static Boolean force_handshake(byte[] s_id){
        output_struct out=new output_struct();
        //  pack_data(ls_out, e_cmd_identify_s, (u1*)s_id, sr_strlen(s_id,15));
        output_struct s_out=new output_struct();
        for(int i=0;i!=s_id.length;i++){
    ...
    public static void start_isp(){
        ...
        int ln_count=pack_data(out.ls_out, (short) ScaleProtocol.ECMD.e_cmd_isp_s.ordinal(), s_out.ls_out, sr_len.sr_len);
    ...
    public static byte[] u1_array_to_byte_array_with_len(Struct.Unsigned8[] s_in,int ln_len){
